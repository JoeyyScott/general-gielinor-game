...
3. Of the options available the one I chose is to clone the link to the repository whilst having "HTTPS" selected.
...
+ favIcon - As stated in my Technologies section I used [FavIcon](https://favicon.io/favicon-converter/) to convert an image I made to an icon and the website also provides the code for the ```<head>``` element to link the icon to the site.
...
+ CSS overscroll - Upon requesting a peer code review a user within the [Code Institute](https://codeinstitute.net/) Slack community mentioned that when scrolling to the bottom of the site, most of the footer content would disappear.
    ...
    + I ended up removing this property entirely and using ```overflow: hidden``` which combined with a slight rescaling of content removed the issue I was facing originally.
